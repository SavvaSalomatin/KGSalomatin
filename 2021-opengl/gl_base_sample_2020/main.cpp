...
Camera camera(float3(0.0f, 10.0f, 30.0f));
...
////////////////////////////
    ...
    GLsizei Galaxy = CreateSphere(1.0f , 250, vaoGalaxy);
////////////////////////////////////////////////////////////////////////////////
 ...
 /////////////////   Сфера
        ...
        {
            model = transpose(mul(translate4x4(float3(-1.0f, 9.0f, 14.0f)), scale4x4(float3(1.0f, 1.0f, 1.0f))));
...
///
        ...
        ///////////////////     Галактика
        glBindVertexArray(vaoGalaxy);
        {
            float h = 30.0f;

            float4x4 modelBlack;
            modelBlack = transpose(mul(translate4x4(float3(0.0f, h, 0.0f)),scale4x4(float3(2.55f,2.55f,2.55f))));
            lambert.SetUniform("model", modelBlack); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Galaxy, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
            float4x4 modelStars;
            for (int i = 1; i < 100; i++)
            {
                modelStars = transpose(mul(rotate_Y_4x4(timer* i * LiteMath::DEG_TO_RAD + i%6/**pow(-1.0f,i)*/), mul(translate4x4(float3(i + 2.0f, h, i + 2.0f)), scale4x4(float3(pow(0.5f/i,0.2f), pow(0.5f / i, 0.2f), pow(0.5f / i, 0.2f))))));
                lambert.SetUniform("model", modelStars); GL_CHECK_ERRORS;
                glDrawElements(GL_TRIANGLE_STRIP, Galaxy, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
            }
        }
